...
1. **Extract** — Retrieve cryptocurrency data via REST API (CoinGecko).
2. **Transform** — 
   - Convert date fields to UTC datetime.
   - Filter by coin symbols based on user input (`*` for all).
   - Drop unnecessary columns.
3. **Validate** — Perform basic data quality checks (null/negative values, data freshness).
4. **Load** — Write the result into a PostgreSQL database table.
